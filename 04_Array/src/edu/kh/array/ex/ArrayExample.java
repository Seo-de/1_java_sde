...
public class ArrayExample {
	...
	public void ex2() {
		...
		// arr.lenght : arr이 참조하고 있는 배열의 길이
		...
		// Stack 영역은 컴파일러가 알아서 초기화 안해줌
		// heap 영역은 컴파일러가 알아서 초기화 해줌
		int a;
		...
		// 1) 인덱스를 이용한 초기화
		arr[0] = 100; // arr int[] 참조형 변수가 참조하고 있는 
		              // 배열의 0번 인덱스에 100을 대입
		arr[1] = 200;
		arr[2] = 300;
		arr[3] = 400;
	...
	public void ex3() {
		...
		// 배열의 주소가 출력됨 -> 왜? heightArr 참조 변수에는 
		// 배열의 시작 주소가 저장되어 있기 때문에
		
		// Arrays.toString(배열명) : 배열에 저장된 모든 값을 한 줄로 간단 출력
		//System.out.println( Arrays.toString(heightArr));
		
		System.out.println();  // 줄 바꿈
		
		double sum = 0; // 합계 (평균 구할 때 사용하려고)
	...
	public void ex4() {
		...
		// Array.toString(arr) : 배열에 저장된 모든 값을 한 줄로 간단히 출력 // 간단히 보여줌.
	...
	public void ex7() {
		...
		for(int i = 1; i<scoreArr.length; i++) {
			...
			if(scoreArr[i] < min) { // 기존 최저점 보다 scoreArr[i] 값이 더 작은 경우
				                    // 유지보수를 위해서 else if 가 아닌 if문 2개를 쓴데
				min = scoreArr[i];
			}
		}
	...
	public void ex8() {
		...
		// 배열 내 데이터 검색
		// 입력 받은 정수가 배열에 존재하머 몇 번 인덱스에 존재하는지 ㅊㄹ력
		// 단, 없다면 "존재하지 않습니다" 출력
	...
	public void ex9() {
		...
		// arr변수에 저장된 배열의 시작 주소를 copyArr에 대입(얕은복사)
		int [] copyArr = arr;
		...
		// 복사본의 값을 변경 -> 원본 값도 변함 == 값을 공유(얕은 복사 특징)
		copyArr[2] = 10000;
