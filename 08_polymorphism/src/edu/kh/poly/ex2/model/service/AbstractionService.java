...
import edu.kh.poly.ex2.model.vo.SDECalculator;
...
//		f1.breath(); // 위에꺼랑 같은 값이 나오는거라 주석처리해줌.
		...
		// 실행 중 오버라이딩한 자식은 breath()로 연결됨 (동적 바인딩)
...
//		KH k1 = new KH(); // 
		...
		for(int i=0; i<arr.length; i++) {
			arr[i].lesson(); 
			// 정적 바인딩 :  void edu.kh.poly.ex2.model.vo.KH.lesson() - 추상 메서드
			// 동적 바인딩 : 
			// arr[0] -> HSH.lesson();
			// arr[1] -> HGD.lesson();
		}
	...
	public void ex3() {
		...
		Calculator cal = new SDECalculator();
