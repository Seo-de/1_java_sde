...
public class StudentView {
		 ...
		 do {
	         ...
	         try {
	        	 ...
	        	 System.out.println();
	         		...
	         		//* 입력이 애초에 잘못돼서 input에 대입이 되지도 못한 채 바로 캐치로 온거야. 그래서 input이 0인거지
	         		// 이걸 input을 -1로 바꿔서 종료되지 않도록 해줄거야
	         		input = -1; // 첫 반복 시 잘못 입력된 경우 while문이 종료되지 않도록 하는 값
	         		sc.nextLine(); // 입력버퍼에 남아있는 잘못된 문자열을 꺼내와 제거 
	         						//* (잘못된게 계속 저장이 되면서 무한루프에 빠지는걸 방지)
	         	}
	...
	public void selectAll() {
		...
		// 향상된 for문
		// - 배열 또는 컬렉션의 모든 요소를 
		//	 처음 부터 끝까지 순차 접근할 때 사용하는 for문 (foreach문)
		
		// for( 요소 하나를 꺼내서 참조할 변수 : 배열 또는 컬렉션 ) { }
		
		for( Student s : stdList ) { // Student에서 하나 뽑아낸게 Student라서 그렇게 썼대....
